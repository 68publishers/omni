...
stan:
	PHP=81 make composer.update
	docker exec 68publishers.omni.81 vendor/bin/phpstan analyse --memory-limit=-1

coverage:
	PHP=81 make composer.update
	docker exec 68publishers.omni.81 vendor/bin/tester -p phpdbg -C -s --coverage ./coverage.xml --coverage-src ./src ./tests
...
ifndef PHP
	...
	docker exec 68publishers.omni.$(PHP) vendor/bin/tester -C -s ./tests
